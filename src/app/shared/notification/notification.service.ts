...
export class NotificationService {
  ...
  infos(messages: string[], title: string) {
  ...
  warns(messages: string[], title: string) {
  ...
  sucesso(msg: string, title: string) {
    this.toastr.success(msg, title);
  }

    send(data: Response) {
        if (isEmpty(data)) return;
        if (!isEmpty(data.infos)) {this.infos(data.infos, ''); }
        if (!isEmpty(data.warns)) {this.warns(data.warns, ''); }
        if (!isEmpty(data.errors)) {this.errors(data.errors, ''); }
    }
